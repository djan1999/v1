...
function generateMenuHTML({ seat, table, menuTitle = "WINTER  MENU", teamNames = "", menuCourses = MENU_DATA, beerChoice = null }) {
  ...
  const PAIRING_COL = { "wp": 0, "na": 1, "os": 2, "premium": 3 };
  const BEER_ALCO   = ["Reservoir Dogs, Crazy Sister", "Nova Gorica, Slovenia"];
  const BEER_NONALC = ["SPENT BREAD KOMBUCHA", "malt, hops"];

  const seatId       = seat.id;
  const pairingLabel = seat.pairing || "";
  const pkey         = PAIRING_MAP[pairingLabel] || null;
  const pcol         = pkey != null ? PAIRING_COL[pkey] : null;
  ...
  const isShort  = (table.menuType || "").toLowerCase() === "short";
  ...
  const allBevNames = [
    ...(seat.cocktails || []).map(b => (b.name || "").toLowerCase()),
    ...(seat.glasses   || []).map(b => (b.name || "").toLowerCase()),
  ];
  const hadSFSC = allBevNames.some(n => n.includes("so fresh") || n.includes("sfsc"));

  // ── Wine column layout ───────────────────────────────────────────────────
  // glasses = by-glass wines on this seat; bottles = table bottle wines
  const glasses     = seat.glasses      || [];
  const tableBottles = table.bottleWines || [];

  const SNACK_END    = 4;
  const BLANK_AFTER  = 11;
  const BEETROOT_IDX = 6;
  const BEER_IDX     = 9;  // CHICKEN GIZZARD (menu) / MUSHROOM DUMPLING (veg)
  const CHEESE_IDX   = 17;

  // Build rightOverride: courseIndex → html string for right column
  const rightOverride = {};

  const fmtWine = w => {
    const sub = [w.producer, w.vintage].filter(Boolean).join("  ");
    return `<b>${w.name}</b>${sub ? `<br>${sub}` : ""}`;
  };

  if (pkey === null) {
    // No pairing — glasses then bottles fill right column from index 0 (top, same as aperitivos)
    const allWines = [...glasses, ...tableBottles];
    allWines.forEach((w, wi) => { rightOverride[wi] = fmtWine(w); });
  } else {
    // Has pairing — show aperitivos in snack rows, then by-glass wines after snacks
    menuCourses.forEach((course, idx) => {
      const isSnackRow = course.is_snack ?? (idx <= SNACK_END);
      if (isSnackRow && course.aperitivo) {
        rightOverride[idx] = `<b>${course.aperitivo.name}</b><br>${course.aperitivo.sub || ""}`;
      }
    });
    glasses.forEach((w, wi) => { rightOverride[SNACK_END + 1 + wi] = fmtWine(w); });
  }

  let rows = "";
  menuCourses.forEach((course, i) => {
    const isSnack = course.is_snack ?? (i <= SNACK_END);
    if (i === BEETROOT_IDX && !hasBeetroot) return;
    if (i === CHEESE_IDX   && !hasCheese)   return;
    if (isShort && isSnack)                 return;

    const d        = (isVeg && course.veg) ? course.veg : course.menu;
    const pairings = [course.wp, course.na, course.os, course.premium];
    let drink      = (pcol != null) ? pairings[pcol] : null;

    // Beer course logic (index 9 — Chicken Gizzard / Mushroom Dumpling)
    if (i === BEER_IDX) {
      const choice = beerChoice || (pkey === "na" ? "nonalc" : pkey != null ? "alco" : (hadSFSC ? "nonalc" : "alco"));
      if (choice === "none") { drink = null; }
      else {
        const b = choice === "nonalc" ? BEER_NONALC : BEER_ALCO;
        drink = { name: b[0], sub: b[1] };
      }
    }

    // Section gaps
    if (!isShort && i === SNACK_END + 1) {
      rows += `<tr><td colspan="3" style="height:3pt"></td></tr>`;
      if (pkey !== null) {
        const PAIRING_LABELS = { wp: "WINE PAIRING", na: "NON-ALC PAIRING", os: "OUR STORY PAIRING", premium: "PREMIUM PAIRING" };
        const pairingLabel = PAIRING_LABELS[pkey] || "WINE PAIRING";
        rows += `<tr><td class="L"></td><td class="M"></td><td class="R" style="padding-bottom:5pt;letter-spacing:0.06em">${pairingLabel}</td></tr>`;
      }
    }
    if (i === BLANK_AFTER + 1)
      rows += `<tr><td colspan="3" style="height:11.5pt"></td></tr>`;

    // Right column: override (glass/bottle) takes priority for no-pairing;
    // for pairing seats, override only applies where drink is empty (snack rows)
    let rightHtml;
    if (rightOverride[i] !== undefined && (pkey === null || !drink)) {
      rightHtml = rightOverride[i];
    } else {
      rightHtml = drink ? `<b>${drink.name}</b><br>${drink.sub || ""}` : "";
    }

    rows += `<tr>
      <td class="L"><b>${d.name}</b><br>${d.sub}</td>
      <td class="M"></td>
      <td class="R">${rightHtml}</td>
    </tr>`;
  });
...
html,body{height:100%;font-family:'RM',monospace;font-size:6.8pt;line-height:1.15;color:#000;}
b{font-weight:700;}
@page{size:A5;margin:14mm 17mm 18mm 17mm;}
@media print{
  html,body{height:100%;}
  #team{position:fixed;bottom:0;left:0;right:0;font-size:6pt;}
}
#header{display:flex;justify-content:space-between;align-items:flex-end;margin-bottom:10mm;}
#title{font-size:13.5pt;font-weight:700;letter-spacing:0.04em;}
#logo img{width:19mm;display:block;}
table{width:100%;border-collapse:collapse;font-size:6.8pt;line-height:1.15;}
td.L{width:47%;vertical-align:top;padding-bottom:2.9pt;}
td.M{width:6%;}
td.R{width:47%;vertical-align:top;padding-bottom:2.9pt;}
#thankyou{margin-top:14.9pt;font-size:6.8pt;}
#team{font-size:6pt;line-height:1.4;margin-top:auto;padding-top:8pt;}
</style>
</head><body>
<div id="header">
  <div id="title">${menuTitle}</div>
  <div id="logo"><img src="data:image/png;base64,${MENU_LOGO}"></div>
</div>
<table>${rows}</table>
<div id="thankyou">Thank you for your visit.</div>
<div id="team"><b>TEAM:</b><br>${teamNames}</div>
</body></html>`;
}
...
function MenuGenerator({ table, menuCourses = MENU_DATA, onClose }) {
  const [teamNames, setTeamNames] = useState("Daniel Polyakov, David Žefran, Djan Meglič, Jakob Zarnik, Joel Gomez, Juan Galindo, Nar Bahadur, Živa Kralj");
...
export default function App() {
  ...
  // ── Menu courses: load from Supabase menu_courses table + realtime ─────────
  useEffect(() => {
    if (!supabase) return;
    let mounted = true;
    ...
    loadCourses();
    const menuChannel = supabase.channel("milka-menu-courses")
      .on("postgres_changes", { event: "*", schema: "public", table: "menu_courses" }, loadCourses)
      .subscribe();
    return () => { mounted = false; supabase.removeChannel(menuChannel); };
  }, []); // eslint-disable-line react-hooks/exhaustive-deps
